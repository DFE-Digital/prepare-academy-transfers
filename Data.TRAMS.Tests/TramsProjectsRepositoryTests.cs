...
namespace Data.TRAMS.Tests
{
    public class TramsProjectsRepositoryTests
    {
        ...
        public class GetProjectsTests : TramsProjectsRepositoryTests
        {
            ...
            {
                _httpClient.Setup(c => c.GetAsync("academyTransferProjects?page=1&count=10")).ReturnsAsync(new HttpResponseMessage
                {
            ...
            {
                _httpClient.Setup(c => c.GetAsync($"academyTransferProjects?page={page}&count=10")).ReturnsAsync(new HttpResponseMessage
                {
                    Content = new StringContent(JsonConvert.SerializeObject(new PagedResult<TramsProjectSummary>(_foundSummaries, 12)))
                });
                ...
                _httpClient.Verify(c => c.GetAsync($"academyTransferProjects?page={page}&count=10"), Times.Once());
            }
        }
